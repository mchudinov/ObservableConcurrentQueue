...
namespace ObservableConcurrentQueue
{
    ...
    public sealed class ObservableConcurrentQueue<T> : ConcurrentQueue<T>
    {
        ...
        public new void Enqueue(T item)
        {
            ...
            this.OnContentChanged(new NotifyConcurrentQueueChangedEventArgs<T>(NotifyConcurrentQueueChangedAction.Enqueue, item));
        }
        ...
        public new bool TryDequeue(out T result)
        {
            ...
            this.OnContentChanged(new NotifyConcurrentQueueChangedEventArgs<T>(NotifyConcurrentQueueChangedAction.Dequeue, result));

            if (this.IsEmpty)
            {
                // Raise Queue empty event
                this.OnContentChanged(new NotifyConcurrentQueueChangedEventArgs<T>(NotifyConcurrentQueueChangedAction.Empty));
            }
        ...
        public new bool TryPeek(out T result)
        {
            ...
            {
                // Raise item dequeued event
                this.OnContentChanged(new NotifyConcurrentQueueChangedEventArgs<T>(NotifyConcurrentQueueChangedAction.Peek, result));
            }
